/**
 * Implements an Othello/Reversi grid along with helpful methods.
 *
 * @author  T.Sergeant
 * @version For AI In-Class Work
 *
 * <p>grid is represented as a 1d array of chars.</p>
*/
import java.util.Scanner;

public class Board
{
	protected char [][] grid;  // the grid
	private int moveCount;    // number of pieces placed


	/**
	 * Start with four pieces in traditional starting configuration.
	 */
	public Board()
	{
		int i,j;
	...
	/**
	 * Determine how many pieces can be captured in the given direction.
	 *
	 * @param otherToken opponent's token to capture
	 * @param i starting row
	 * @param j starting column
	 * @param ioff change in row (+1, 0, -1)
	 * @param joff change in column (+1, 0, -1)
	 * @return number of pieces that can be captured
	 */
	private int countCaptures(char otherToken, int i, int j, int ioff, int joff)
	{
		int a,b,n=0;

		for (a=i+ioff, b=j+joff; a>=0 && a<8 && b>=0 && b<8 && grid[a][b]==otherToken; a+=ioff, b+=joff)
			n++;
		if (a<0 || a>7 || b<0 || b>7 || grid[a][b]==' ') return 0;
		return n;
	}
